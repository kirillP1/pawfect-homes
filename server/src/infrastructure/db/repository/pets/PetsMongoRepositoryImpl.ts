...
export class PetsMongoRepositoryImpl implements PetRepository {
  ...
  async update(dto: UpdatePetDto): Promise<Pet> {
    const { id, ...input } = dto
    const entity = await PetEntity.findByIdAndUpdate(
      id, 
      input, 
      { new: true, runValidators: true }
    ).select('name species breed age gender adoptionFee imageUrl description shelterId isAdopted createdAt updatedAt')
  ...
  async getAll(): Promise<Pet[]> {
    const entities = await PetEntity.find()
      .select('name species breed age gender adoptionFee imageUrl description shelterId isAdopted createdAt updatedAt')
    return entities.map(PetMapper.toDomain)
  }

  async getById(id: string): Promise<Pet> {
    const entity = await PetEntity.findById(id)
      .select('name species breed age gender adoptionFee imageUrl description shelterId isAdopted createdAt updatedAt')
  ...
  async remove(id: string): Promise<Pet> {
    const entity = await PetEntity.findByIdAndDelete(id)
      .select('name species breed age gender adoptionFee imageUrl description shelterId isAdopted createdAt updatedAt')
